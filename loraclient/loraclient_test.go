...
import (
	...
	"github.com/lora/go-lora/consensus/ethash"
	"github.com/lora/go-lora/core"
	"github.com/lora/go-lora/core/types"
	"github.com/lora/go-lora/crypto"
	"github.com/lora/go-lora/eth"
	"github.com/lora/go-lora/eth/ethconfig"
	"github.com/lora/go-lora/node"
	"github.com/lora/go-lora/params"
	"github.com/lora/go-lora/rpc"
)

// Verify that Client implements the lora interfaces.
var (
	_ = lora.ChainReader(&Client{})
	_ = lora.TransactionReader(&Client{})
	_ = lora.ChainStateReader(&Client{})
	_ = lora.ChainSyncReader(&Client{})
	_ = lora.ContractCaller(&Client{})
	_ = lora.GasEstimator(&Client{})
	_ = lora.GasPricer(&Client{})
	_ = lora.LogFilterer(&Client{})
	_ = lora.PendingStateReader(&Client{})
	// _ = lora.PendingStateEventer(&Client{})
	_ = lora.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new lora service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == lora.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != lora.NotFound {
		t.Fatal("error should be lora.NotFound")
	}
}
